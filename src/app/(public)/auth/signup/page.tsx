...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emailExists = await UserService.findPageCustomize({
        filter: { email: formData.email },
        limit: 1,
        offset: 0,
      });

      console.error("Email exists:", formData.email, emailExists);
      ...
      const phoneExists = await UserService.findPageCustomize({
        filter: { email: formData.email },
        limit: 1,
        offset: 0,
      });

      if (phoneExists.data && phoneExists.data.length > 0) {
        toast.error("Phone already exists", {
          description: "Please use a different phone or sign in.",
        });
        setErrors((prev) => ({
          ...prev,
